...
## 摘要
神经重建通常以牺牲结构来换取保真度，导致生成的网格密集且无结构，具有不规则的拓扑结构和模糊的部件边界，从而阻碍了编辑、动画制作和下游资产的复用。我们提出了 DualPrim，这是一个紧凑且结构化的三维重建框架。与仅采用加法的隐式或原语方法不同，DualPrim 使用正向和负向超二次曲面来表示形状：前者构建基础，而后者通过可微算子雕刻局部体积，从而实现对孔洞和凹陷的拓扑感知建模。这种加减法设计在不牺牲紧凑性或可微性的情况下增强了表达能力。我们将 DualPrim 嵌入到体素可微渲染器中，实现了从多视图图像进行端到端学习，并通过闭式布尔差集运算实现无缝网格导出。实验表明，DualPrim 达到了最先进的精度，并生成了紧凑、结构化且可解释的输出，比仅采用加法的替代方案更好地满足了下游需求。

...
## 摘要
基于切片的体积成像应用广泛，它要求表示方法在进行强力压缩的同时，能够保留内部结构以供分析。我们引入了 GaussianPile，通过将 3D 高斯泼溅（3D Gaussian splatting）与成像系统感知的聚焦模型相结合来应对这一挑战。我们提出的方法引入了三项关键创新：(i) 一种切片感知堆叠策略，通过放置各向异性 3D 高斯来建模跨切片贡献；(ii) 一个可微投影算子，用于编码成像采集系统的有限厚度点扩散函数（PSF）；(iii) 一个紧凑的编码和联合优化流水线，可同时重建和压缩高斯集合。我们基于 CUDA 的设计在保留高斯原语的压缩和实时渲染效率的同时，保留了高频内部体积细节。在显微镜和超声数据集上的实验表明，我们的方法降低了存储和重建成本，维持了诊断保真度，并实现了快速 2D 可视化以及 3D 体素化。在实践中，它在短短 3 分钟内即可提供高质量结果，比基于 NeRF 的方法快达 11 倍，并实现了相对于体素网格一致的 16 倍压缩，为基于切片的体积数据集的可部署压缩和探索提供了一条实用路径。

...
tldr: 针对分布式无人机低空高斯泼溅（LAGS）三维重建中的资源分配效率问题，本文提出了一种基于分组异构图神经网络（GW-HGNN）的优化方案。该方法通过显式建模不同视角图像组对重建质量的非均匀贡献，在保证数据保真度的同时降低传输成本。实验证明，GW-HGNN在渲染指标上优于现有基准，且计算速度比传统求解器快100倍，支持毫秒级实时部署。
selection_source: fresh_fetch
motivation: 现有的低空资源分配方案忽略了不同视角图像对重建质量贡献的差异，导致通信开销与重建精度之间缺乏有效平衡。
method: 提出GW-HGNN模型，将重建损失与通信约束转化为图学习成本，利用双层消息传递机制优化资源分配。
result: 在多个真实数据集上，该方法在PSNR等渲染指标上显著优于基准，且推理速度比传统MOSEK求解器快约100倍。
conclusion: GW-HGNN通过异构图学习实现了重建质量与传输成本的自动平衡，为实时低空三维重建提供了高效的资源分配方案。
---

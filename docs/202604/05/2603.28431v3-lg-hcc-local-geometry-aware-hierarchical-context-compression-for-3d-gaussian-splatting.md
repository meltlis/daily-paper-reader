...
## 摘要
虽然 3D 高斯泼溅 (3DGS) 能够实现高保真实时渲染，但其巨大的存储开销严重阻碍了实际部署。最近基于锚点 (anchor-based) 的 3DGS 压缩方案通过一些先进的上下文模型减少了高斯冗余。然而，它们忽略了显式的几何依赖关系，导致结构退化和次优的率失真性能。在本文中，我们提出了一种用于 3DGS 的局部几何感知分层上下文压缩框架 (LG-HCC)，该框架将锚点间的几何相关性纳入锚点剪枝和熵编码中，以实现紧凑表示。具体而言，我们引入了一种邻域感知锚点剪枝 (NAAP) 策略，该策略通过加权邻域特征聚合评估锚点重要性，然后将低贡献锚点合并到显著邻居中，从而产生一个紧凑且几何一致的锚点集。此外，我们进一步开发了一种分层熵编码方案，其中通过轻量级几何引导卷积 (GG-Conv) 算子利用从粗到细的先验，以实现空间自适应上下文建模和率失真优化。大量实验表明，LG-HCC 有效缓解了结构保持问题，在 Mip-NeRF360 数据集上与 Scaffold-GS 基准相比，在实现卓越几何完整性和渲染保真度的同时，将存储空间减少了高达 30.85 倍。

...
## 摘要
现代场景重建方法（如 3D Gaussian Splatting）能够以实时速度实现照片级真实感的新视角合成。然而，与传统的人工创作 3D 资产相比，由于与这些表示形式进行交互较为困难，它们在交互式图形应用中的采用仍然受限。尽管先前的工作尝试对这些模型施加语义分解，但在分割质量和跨视角一致性方面仍面临重大挑战。为了解决这些局限性，我们提出了 Semantic Foam，它将最近提出的 Radiant Foam 表示扩展到语义分解任务中。我们的方法利用了 Radiant Foam 体积 Voronoi 网格固有的空间结构，并使用在单元级别定义的显式语义特征场对其进行了增强。这种设计实现了直接的空间正则化，提高了跨视角的一致性，并减轻了由遮挡和不一致监督引起的伪影，而这些是基于点表示的常见问题。实验结果表明，与 Gaussian Grouping 和 SAGA 等最先进的方法相比，我们的方法实现了更优越的对象级分割性能。项目页面：http://semanticfoam.github.io/
